...
namespace ParkingManagementSystem
{
    ...
    public class ParkingLot : IDisposable
    {
        ...
        private int FindAvailablePWDSlot()
        {
            // Search for available PWD slot (9 and 10)
            for (int i = 8; i < 10; i++)
            {
            ...
            return -1; // No available PWD slot found
        }

        private bool ParkRegularVehicle(string fullName, string vehicleType, string vehicleNumber)
        {
        ...
        public bool ParkVehicle(string fullName, string vehicleType, string vehicleNumber)
        {
            ...
            if (isPWD == "yes")
            {
                int pwdSlotNumber = FindAvailablePWDSlot();
                if (pwdSlotNumber != -1)
                {
                    // Park the PWD vehicle in the available PWD slot
                    ParkingSlot pwdSlot = slots[pwdSlotNumber - 1];
                    pwdSlot.IsOccupied = true;
                    pwdSlot.FullName = fullName;
                    pwdSlot.VehicleType = vehicleType;
                    pwdSlot.VehicleNumber = vehicleNumber;
                    pwdSlot.EntryTime = DateTime.Now;
                    ...
                    using (MySqlCommand updateCommand = new MySqlCommand(updateQuery, connection))
                    {
                        ...
                        updateCommand.Parameters.AddWithValue("@vehicleType", vehicleType);
                        updateCommand.Parameters.AddWithValue("@vehicleNumber", vehicleNumber);
                        updateCommand.Parameters.AddWithValue("@entryTime", pwdSlot.EntryTime);
                        updateCommand.Parameters.AddWithValue("@slotNumber", pwdSlotNumber);

                        updateCommand.ExecuteNonQuery();
                    }

                    Console.WriteLine($"Vehicle {vehicleNumber} (Driver: {fullName}) parked at PWD slot {pwdSlotNumber}.");
                    return true;
                }
                else
                {
                    Console.WriteLine("No available PWD slots. Parking in a regular slot.");
                    return ParkRegularVehicle(fullName, vehicleType, vehicleNumber);
                }
            }
            else
            {
                    ...
                    do
                    {
                        ...
                        switch (choice)
                        {
                            case "1":
                                ...
                                parkingLot.ParkVehicle(fullName, vehicleType, vehicleNumber);
                                break;

                            case "2":
                                Console.Write("Enter vehicle number to take: ");
                                string vehicleNum = Console.ReadLine();

                                parkingLot.LeaveParking(vehicleNum);
                                break;
